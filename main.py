    ...
    def creuser(i, j, P, A):
        ...
        if P.p2[i][j] == 1:
            A.pt[i][j] = tk.Button(A.plateau, width=5, height=1, text="BOUM", bg="red",
                                   command=partial(Game.is_mine, i, j, P, A))
            A.pt[i][j].grid(row=i, column=j)

        elif (P.p2[i][j] == 0) and (P.p3[i][j] != 0):
            A.pt[i][j] = tk.Button(A.plateau, width=5, height=1, text=P.p3[i][j], bg="blue",
                                   command=partial(Game.is_mine, i, j, P, A))
            A.pt[i][j].grid(row=i, column=j)
    ...
    def recur(i, j, P, A):
        if P.p2[i][j] == 0:  # si pas de mine sur ij
            ...
            for k in range(-1, 2, 1):
                for h in range(-1, 2, 1):
                    try:
                        if (P.p4[i + k][j + h] == 0) and (P.p3[i + k][j + h] == 0) and (i + k >= 0) and (
                                j + h >= 0) and ((k != 0) or (h != 0)):  # si pas creuser et si pas de mines autour
                            try:
        ...
        for i in range(x):
            for j in range(x):
                self.pt[i][j] = tk.Button(self.plateau, width=5, height=1, text="",
                                          command=partial(Game.is_mine, i, j, P, self))
